                    ...
                    <form action="tutor-deletecourse.php" method="GET">
                        ...
                        '<div class="form-group">
                             <label class="control-label col-sm-2" for="choosenSubjects">Subjects</label>
                                <div class="col-sm-10">';
                                //displaying tutor's courses as checkboxes
                                for ($x; $x <= $count4; $x++){                
 ...
 <div class="modal fade" id="editModal3" role="dialog">
    <div class="modal-dialog">
            ...
            <div class="modal-body">
               <!-- Modal Body -->
                <form action="tutor-addcourses.php" method="POST">
                    <?php        
                     $sqlclass="SELECT * FROM `onlinetuitionmanagementsystem`.`class` WHERE class_id NOT IN (SELECT M.class_id from tutor_courses AS E join courses AS M ON E.course_id=M.course_id where tutor_id='$user') ORDER BY class_id DESC;";
                    ...
                    <div class="form-group">'; 
                        for ($xclass; $xclass <= $countclass; $xclass++){
                            ...
                            echo '  <h4>Choose Only one Subject from one class</h4> 
                            <h4>  Subjects for '.$rowclass[$xclass-1]["class_id"].'</h4>';
                            //selecting courses which have not already been selected by the tutor
                            $addcourse="SELECT course_id,course_name,class_id FROM courses WHERE class_id='$class';";
                            $run_queryadd = mysqli_query($conn,$addcourse);
	                        $countadd = mysqli_num_rows($run_queryadd);
                            $rowadd=[]; 
                             
                        if($countadd==0){
                            echo' <h4 class="modal-title">Already enrolled in all classes.You cannot choose any more subjects. </h4>
                    </div></div>';
                            } 
                        else{  
        ...
        <div>
                    ...
                    <h3 style="color: rgb(9, 13, 66);">ACADEMIC INFORMATION</h3>
